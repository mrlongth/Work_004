...
namespace myDLL
{
    public class cExcelReader
    {
        ...
        #region "Variables"
        private string _strCriteria = string.Empty;
        private int[] _PKCol;
        ...
        public string ColName(int intCol)
        {
            ...
            if (intCol < 26)
            {
                sColName = Convert.ToString(Convert.ToChar((Convert.ToByte(Convert.ToChar('A')) + intCol)));
            }
            else
            {
                int intFirst = (Convert.ToInt32(intCol) / 26);
                int intSecond = (Convert.ToInt32(intCol) % 26);
                sColName = Convert.ToString(Convert.ToByte(Convert.ToChar('A')) + intFirst);
                sColName += Convert.ToString(Convert.ToByte(Convert.ToChar('A')) + intSecond);
            }
            return sColName;
        }
        ...
        private bool SetSheetQuerySelect()
        {
            try
            {
                ...
                _oleCmdSelect = new OleDbCommand(("SELECT * FROM [" + _strSheetName + "$") + _strSheetRange + "]  Where 1=1  " + _strCriteria, _oleConn);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private OleDbDataAdapter SetSheetQueryAdapter(DataTable dt)
        {
            ...
            try
            {
                ...
                for (int iPK = 0; iPK <= PKCols.Length - 1; iPK++)
                {
            ...
            try
            {
                if (PKCols != null)
                {
                    ...
                    if (PKCols.Length > 0)
                    {
                        ...
                        for (int i = 0; i <= PKCols.Length - 1; i++)
                        {
        ...
        public DataTable GetTable(string strTableName, string strCriteria)
        {
            try
            {
                //Open and query 
                ...
                if (this.Headers == false)
                {
                    if (_strSheetRange.IndexOf(":") > 0)
                    {
                        ...
                        for (int intI = 0; intI <= dt.Columns.Count - 1; intI++)
                        {
        ...
        public void InsertTable(string strCmd)
        {
            try
            {
                //Open and query 
